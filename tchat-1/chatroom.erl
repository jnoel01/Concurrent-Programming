...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	Keep = fun(X,_Y) ->
		X =/= ClientPID end,
		Clients = maps:keys(maps:filter(Keep,State#chat_st.registrations)),
		{ ok, ClientNick } = maps:find(ClientPID,State#chat_st.registrations),
		UpdatedNick = fun(X) ->
			X!{ request,self(), Ref, { incoming_msg,ClientNick,State#chat_st.name,Message }} end, lists:foreach(UpdatedNick,Clients),
